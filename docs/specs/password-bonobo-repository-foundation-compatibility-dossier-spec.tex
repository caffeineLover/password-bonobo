...
\item
  Demonstrated through a tracked exact generator that paired documentation is reproducible and generated PDFs remain
  uncommitted.
\item
  Recorded dependencies, build and documentation tools, workflow actions, and repository assets in an automatically
...
\item
  The empty \nolinkurl{bonobo_core} package boundary and its foundation tests.
\item
...
\tightlist
\item
  \nolinkurl{sources/gorilla.tcl} for application workflows and user-visible behavior.
\item
  \nolinkurl{sources/pwsafe/*.tcl} for supported data concepts and compatibility behavior.
\item
  \nolinkurl{sources/help.txt}, message catalogs, and upstream README material for user-facing contracts.
\item
  \nolinkurl{unit-tests/**/*.test} and \nolinkurl{unit-tests/**/*.tcl} for behavioral examples and edge cases.
\item
...
\item
  \nolinkurl{Modernized}: the behavior is required but its interaction design will change.
\item
...
\item
  \nolinkurl{Unverified}: evidence is insufficient and additional research is required.
\end{itemize}
...
The repository will target CPython \nolinkurl{>=3.14,<3.15} for the first development baseline. A version-floor change
requires platform and dependency review because the same core is intended for desktop, Android, and iOS.

The initial package boundary will be \nolinkurl{src/bonobo_core/}. It will contain no vault implementation in this
subproject. The package will be marked typed and expose only version metadata.
...
\item
  Run the Python structure-policy and clean-room compatibility-contract checks.
\item
  Verify provenance-ledger freshness.
\item
...
\item
  Build the Python wheel and source distribution, assert exact GPL wheel metadata and license bytes, and require the GPL
  text and package typing marker in both archives.
\end{enumerate}
...
Bonobo-authored code will use SPDX identifier \nolinkurl{GPL-3.0-or-later}. The repository will contain the complete
