...
The reviewed default outer encrypted-file ceiling is 4,296,015,872 bytes (2^32 plus 1 MiB).  This admits one maximum
uint32 payload frame plus the fixed envelope, mandatory schema fields, and bounded metadata overhead while keeping
streamed snapshot disk and I/O consumption finite.  Callers may lower this ceiling but cannot disable or widen it.
